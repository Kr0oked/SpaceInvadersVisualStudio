...
/* MW: added __declspec(noreturn) for MSVC7 */
...
/* MW:PBuffer support */
#define GLUT_DISPLAY_MODE_POSSIBLE_PBUFFER	((GLenum) 401)
#define GLUT_PBUFFER_SUPPORTED ((GLenum) 402)
...
/* allows interactively redrawing windows (independent of main loop */
GLUTAPI void APIENTRY glutUpdateAllWindows(void);
GLUTAPI void APIENTRY glutMainLoop(void);

/* GLUT window sub-API. */
GLUTAPI int APIENTRY glutCreateWindow(const char *title);
/* Create a Pbuffer (only on Win32) */
GLUTAPI int APIENTRY glutCreateBuffer(const char *title);
#if defined(_WIN32) && !defined(GLUT_DISABLE_ATEXIT_HACK)
...
/* determines what happens if close button is pressed (WIN32 only) */
GLUTAPI void APIENTRY glutCloseFunc(void (GLUTCALLBACK *func)(void));
